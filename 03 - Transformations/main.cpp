...
//############################################################################################################################################
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(1.10f, 0.0f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço esq
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
                      * Matrix_Rotate_Z(g_AngleZ)  // TERCEIRO rotação Z de Euler
                      * Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                      * Matrix_Rotate_X(g_AngleX); // PRIMEIRO rotação X de Euler
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.2f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do braço esq
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                            ...
                            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                                ...
                                PushMatrix(model); // Guardamos matriz model atual na pilha
                                    ...
                                    PushMatrix(model); // Guardamos matriz model atual na pilha
                                        model = model * Matrix_Scale(0.2f, 0.2f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço direito
                                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                        DrawCube(render_as_black_uniform); // #### MÃO ESQUERDA // Desenhamos o antebraço direito
                                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
//############################################################################################################################################
                            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        ...
        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
         ...
         PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(0.0f, 0.45f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação
                      * Matrix_Rotate_Z(-g_ForearmAngleZ)  // TERCEIRO rotação Z de Euler
                      * Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                      * Matrix_Rotate_X(g_ForearmAngleX); // PRIMEIRO rotação X de Euler
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.4f, 0.4f, 0.4f); // Atualizamos matriz model (multiplicação à direita) com um escalamento
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### CABEÇA // Desenhamos
        PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
//############################################################################################################################################
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(g_TorsoPositionX - 1.2f, g_TorsoPositionY - 0.05f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação
            PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.7f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### COXA ESQUERDA // Desenhamos
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente

                        PushMatrix(model); // Guardamos matriz model atual na pilha
                            model = model * Matrix_Translate(0.0f, -0.75f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação
                            PushMatrix(model); // Guardamos matriz model atual na pilha
                                model = model * Matrix_Scale(0.25f, 0.7f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                DrawCube(render_as_black_uniform); // #### CANELA ESQUERDA // Desenhamos
                            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente

                                PushMatrix(model); // Guardamos matriz model atual na pilha
                                    model = model * Matrix_Translate(0.0f, -0.75f, 0.1f); // Atualizamos matriz model (multiplicação à direita) com a translação

                                    PushMatrix(model); // Guardamos matriz model atual na pilha
                                        model = model * Matrix_Scale(0.2f, 0.1f, 0.4f); // Atualizamos matriz model (multiplicação à direita) com um escalamento
                                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                        DrawCube(render_as_black_uniform); // #### PÉ ESQUERDO // Desenhamos
                                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
//############################################################################################################################################
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(g_TorsoPositionX - 0.8f, g_TorsoPositionY - 0.05f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação
            PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.7f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                            ...
                            PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                                ...
                                PushMatrix(model); // Guardamos matriz model atual na pilha
                                    ...
                                    PushMatrix(model); // Guardamos matriz model atual na pilha
                                        model = model * Matrix_Scale(0.2f, 0.1f, 0.4f); // Atualizamos matriz model (multiplicação à direita) com um escalamento
                                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                        DrawCube(render_as_black_uniform); // #### PÉ DIREITO // Desenhamos
                                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
