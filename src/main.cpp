...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "INF01047 - 277765 - Yasmin Katerine Beer Zebrowski", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
void ComputeNormals(ObjModel* model)
{
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape)
    {
        ...
        for (size_t triangle = 0; triangle < num_triangles; ++triangle)
        {
            ...
            glm::vec3 v = glm::vec3(c.x - b.x, c.y - b.y, c.z - b.z);
            glm::vec3 normalTriangle = cross(u, v);

            const glm::vec4  n = glm::vec4(normalTriangle.x, normalTriangle.y, normalTriangle.z, 0.0f); //AQUI
