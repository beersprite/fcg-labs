...
float g_CameraDistanceX = 3.50f;
float g_CameraDistanceY = 0.0f;
float g_CameraDistanceZ = 0.0f;
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00277765 - Yasmin Katerine Beer Zebrowski", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 camera_position_c  = glm::vec4(g_CameraDistanceX, g_CameraDistanceY, g_CameraDistanceZ, 1.0f); // Ponto "c", centro da câmera
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if (key == GLFW_KEY_W && action == GLFW_REPEAT) // W: frente
    {
        g_CameraDistanceY -= (camera_step * sin_g_CameraPhi);
        g_CameraDistanceZ -= (camera_step * cos_g_CameraPhi * cos_g_CameraTheta);
        g_CameraDistanceX -= (camera_step * cos_g_CameraPhi * sin_g_CameraTheta);
    }
    if (key == GLFW_KEY_S && action == GLFW_REPEAT)  // A: esquerda
    {
        g_CameraDistanceY += (camera_step * sin_g_CameraPhi);
        g_CameraDistanceZ += (camera_step * cos_g_CameraPhi * cos_g_CameraTheta);
        g_CameraDistanceX += (camera_step * cos_g_CameraPhi * sin_g_CameraTheta);
    }
    if (key == GLFW_KEY_A && action == GLFW_REPEAT) // S: trás
    {
        g_CameraDistanceX -= (camera_step * cos_g_CameraPhi * cos_g_CameraTheta);
        g_CameraDistanceZ += (camera_step * cos_g_CameraPhi * sin_g_CameraTheta);
    }
    if (key == GLFW_KEY_D && action == GLFW_REPEAT) // D: direita
    {
        g_CameraDistanceX += (camera_step * cos_g_CameraPhi * cos_g_CameraTheta);
        g_CameraDistanceZ -= (camera_step * cos_g_CameraPhi * sin_g_CameraTheta);
    }
