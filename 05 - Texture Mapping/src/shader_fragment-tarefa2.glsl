...
void main()
{
    ...
    if ( object_id == SPHERE )
    {
        ...
        //   constante M_PI
        //   variável position_model (coordenadas locais do objeto)

        vec4 bbox_center = (bbox_min + bbox_max) / 2.0;

        float radius = 1;
        vec4 p_sphere =  bbox_center + radius * normalize(position_model - bbox_center) - bbox_center;

        float theta = atan(p_sphere.x, p_sphere.z);
        float phi = asin(p_sphere.y);
